...
'''
s,n=re.subn(r"if \(\$action === 'conversation_create'.*?(?=\nif \(\$action ===)", '', s, flags=re.S)
if n<1: raise SystemExit('no conversation blocks removed')
marker="if ($action === 'messages'"
if marker not in s: raise SystemExit('messages marker missing')
s=s.replace(marker, block+marker, 1)
p.write_text(s)
print('removed',n,'conversation blocks')
