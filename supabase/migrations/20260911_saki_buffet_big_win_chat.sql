...
as $$
declare v_profile record; v_room record;
begin
  if new.payout < 1000000 or coalesce(old.payout,0) >= 1000000 then return new; end if;
  select username,avatar_url into v_profile from public.profiles where id=new.user_id;
  for v_room in select id,owner_id from public.rooms where coalesce(is_active,true)=true loop
    insert into public.room_messages(room_id,sender_id,body,message_type,payload)
    values(
      v_room.id,
      v_room.owner_id,
      'فوز كبير في لعبة بوفيه الأطعمة',
      'buffet_big_win',
      jsonb_build_object(
        'user_id',new.user_id,
        'username',coalesce(v_profile.username,'مستخدم'),
