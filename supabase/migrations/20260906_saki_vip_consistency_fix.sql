-- profiles + expiry are the source of truth for active VIP membership.
update public.saki_account_modules m
set vip_level = case when p.vip_level > 0 and p.vip_expires_at > now() then p.vip_level else 0 end,
    vip_label = case when p.vip_level > 0 and p.vip_expires_at > now() then 'VIP ' || p.vip_level else 'عضو جديد' end,
    updated_at = now()
...
begin
  ...
  update public.saki_account_modules
     set gold_coins = gold_coins - cost, vip_level = p_level,
