-- Server-authoritative luck gifts: 38% daily win chance and value-only multipliers.
alter table public.gift_announcements
  add column if not exists event_type text not null default 'gift',
  add column if not exists multiplier integer not null default 1,
  add column if not exists reward_gold bigint not null default 0,
  add column if not exists gift_price bigint not null default 0;

create table if not exists public.luck_daily_settings (
  luck_date date primary key default current_date,
  win_percent smallint not null default 38 check (win_percent between 0 and 100),
  updated_at timestamptz not null default now()
);
alter table public.luck_daily_settings enable row level security;
drop policy if exists luck_daily_settings_read on public.luck_daily_settings;
create policy luck_daily_settings_read on public.luck_daily_settings for select to authenticated using (true);
insert into public.luck_daily_settings(luck_date, win_percent)
values (current_date, 38)
on conflict (luck_date) do nothing;

-- Results are delivered through the existing realtime announcement table and room chat.
...
begin
  ...
  -- 38% is the server-controlled chance of any multiplier. The multiplier is always applied only to gift value.
  v_roll := random() * 100;
  if v_roll < v_win_percent then
    v_roll := random() * 10000;
    if v_roll < 6000 then v_multiplier := 2;
    elsif v_roll < 8000 then v_multiplier := 5;
    elsif v_roll < 9000 then v_multiplier := 10;
    elsif v_roll < 9500 then v_multiplier := 20;
    elsif v_roll < 9800 then v_multiplier := 50;
    elsif v_roll < 9950 then v_multiplier := 100;
    elsif v_roll < 9990 then v_multiplier := 500;
    else v_multiplier := 1000;
    end if;
    v_reward := v_total * v_multiplier;
    insert into public.saki_account_modules(user_id,gold_coins) values(p_recipient_id,v_reward)
      on conflict(user_id) do update set gold_coins=public.saki_account_modules.gold_coins+excluded.gold_coins,updated_at=now();
  end if;

  insert into public.room_gifts(room_id,sender_id,recipient_id,gift_id,quantity,total_price,recipient_diamonds)
    values(p_room_id,auth.uid(),p_recipient_id,p_gift_id,p_quantity,v_total,0);
  ...
  v_payload := jsonb_build_object(
    ...
    'room_id',p_room_id, 'room_name',r.name, 'gift_price',v_total,
    'multiplier',v_multiplier, 'reward_gold',v_reward, 'win_percent',v_win_percent);
  ...
  if v_multiplier > 1 then
    insert into public.gift_announcements(room_id,sender_id,recipient_id,gift_id,total_price,recipient_diamonds,event_type,multiplier,reward_gold,gift_price)
      values(p_room_id,auth.uid(),p_recipient_id,p_gift_id,v_total,v_reward,'luck_multiplier',v_multiplier,v_reward,v_total)
      returning id into v_announcement;
    insert into public.room_messages(room_id,sender_id,body,message_type,payload)
      values(p_room_id,auth.uid(),format('%s حصل على ضعف ×%s وحصل على %s عملة ذهبية',recipient_profile.username,v_multiplier,v_reward),'luck_multiplier',v_payload);
  else
    v_announcement := null;
    insert into public.room_messages(room_id,sender_id,body,message_type,payload)
...
-- Realtime events are visible in every room.
do $$ begin
  if not exists (select 1 from pg_publication_tables where pubname='supabase_realtime' and schemaname='public' and tablename='gift_announcements') then
